...
var $ = require('jquery');
require('./hdfs-node-settings.js');

if (!window.contextVars.currentUser.hasAuth) {

    $(document).ready(function () {

        $(window.contextVars.hdfsSettingsSelector).on('submit', function (evt) {
            evt.preventDefault();
            var $this = $(this);
            ...
            $.ajax({
                ...
                msgElm.text('Hdfs access keys loading...')
    ...
    $(document).ready(function () {
        $(window.contextVars.hdfsSettingsSelector).on('submit', AddonHelper.onSubmitSettings);
    });
}
